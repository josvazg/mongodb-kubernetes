...
func TestVerifyDockerfileSources_AllExist(t *testing.T) {
	tmp := t.TempDir()
	src := filepath.Join(tmp, "Dockerfile")
	if err := os.WriteFile(src, []byte("FROM scratch\n"), 0o600); err != nil {
		t.Fatalf("write src: %v", err)
	}
	plan := []DockerfileCopy{
		{Src: src, Dst: filepath.Join(tmp, "out", "a")},
		{Src: src, Dst: filepath.Join(tmp, "out", "b")}, // duplicate Src — must not double-stat
	}
	if err := VerifyDockerfileSources(plan); err != nil {
		t.Fatalf("VerifyDockerfileSources: %v", err)
	}
}

func TestVerifyDockerfileSources_ListsAllMissing(t *testing.T) {
	tmp := t.TempDir()
	present := filepath.Join(tmp, "exists.Dockerfile")
	if err := os.WriteFile(present, []byte("FROM scratch\n"), 0o600); err != nil {
		t.Fatalf("write src: %v", err)
	}
	missingA := filepath.Join(tmp, "missing-a.Dockerfile")
	missingB := filepath.Join(tmp, "missing-b.Dockerfile")
	plan := []DockerfileCopy{
		{Src: missingA, Dst: filepath.Join(tmp, "out", "a")},
		{Src: present, Dst: filepath.Join(tmp, "out", "p")},
		{Src: missingB, Dst: filepath.Join(tmp, "out", "b")},
	}
	err := VerifyDockerfileSources(plan)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	for _, want := range []string{missingA, missingB} {
		if !strings.Contains(err.Error(), want) {
			t.Errorf("error should mention missing %q\nerror: %v", want, err)
		}
	}
	if strings.Contains(err.Error(), present) {
		t.Errorf("error should NOT mention present %q\nerror: %v", present, err)
	}
}
