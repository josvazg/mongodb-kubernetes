...
// PreflightInputs are the conditions a release subcommand checks before
...
// the CLI flag) and validated here as a pure function.
...
func (p PreflightInputs) Validate() error {
	...
	if p.Branch == "master" {
		return errors.New("release commands cannot run on master; create a feature branch first")
	}
