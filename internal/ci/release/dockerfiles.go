...
// VerifyDockerfileSources stat()s every distinct Src in the plan and returns
// an error listing all missing sources at once. Run before bumping or copying
// so the user sees the full list of "you forgot to copy these from the OM-bump
// commit" cases up front instead of discovering them mid-release.
func VerifyDockerfileSources(plan []DockerfileCopy) error {
	...
	return fmt.Errorf("missing source Dockerfiles (copy them from the corresponding upstream commit before re-running):\n  - %s", strings.Join(missing, "\n  - "))
}
