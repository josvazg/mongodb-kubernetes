...
With --dry-run the verification runs but no tag is created or pushed.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			tagArgs := []string{"tag", gitTag, candidate.Commit}
			if force {
				tagArgs = []string{"tag", "-f", gitTag, candidate.Commit}
			}
			if err := r.Exec(ctx, "git", tagArgs...); err != nil {
	...
	cmd.Flags().BoolVar(&force, "force", false, "overwrite the tag if it already exists")
