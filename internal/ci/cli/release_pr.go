...
func runReleasePRCmd(ctx context.Context, version string, draft, dryRun bool, repoOverride string) error {
	...
	// Verify source Dockerfiles up front, before any worktree changes. Catches
	// the common "you forgot to copy mongodb-agent / ops-manager Dockerfiles
	// from the OM-bump commit" case before bump leaves a half-baked release.json.
	if err := verifyDockerfileSources(r.LogOut, version, defaultDockerfilesDest); err != nil {
		return err
	}
